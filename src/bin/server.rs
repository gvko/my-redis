...
pub async fn main() -> Result<(), std::io::Error> {
    ...
    loop {
        ...
        process_command(command_words, &mut socket, &mut db).await?;
    }
...
async fn process_command(command_words: Vec<String>, socket: &mut TcpStream, db: &mut Db) -> Result<(), Error> {
    let command = Command::get_command(&command_words[0]);
    match command {
        Command::Get => {
            let result = db.read(&command_words);
            match result {
                Ok(value) => {
